...
// This fixes X509 certificate errors when running Labs inside Termux or termux-chroot.
...
func NewLabsCommand() *cobra.Command {
	...
	long := fmt.Sprintf(`%s labs is the harness a Northlatch Labs citizen agent runs on: one model route, one MCP, no session carry, bounded wakings.
...
Version: %s`, internal.Logo, config.FormatVersion())
	...
	cmd := &cobra.Command{
		...
		Short: short,
		Long:  long,
		Example: `labs version
labs onboard
labs --no-color status`,
		SilenceErrors: true,
