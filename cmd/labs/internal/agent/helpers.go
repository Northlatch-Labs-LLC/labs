...
func agentCmd(message, model string, debug bool) error {
	...
	// The alias in agents.defaults.model_name stays the alias. Upstream replaced
	// it here with the resolved model id, which is how "weir-gw" became "auto"
	// and matched the wrong entry downstream.
...
func interactiveMode(agentLoop *agent.AgentLoop, sessionKey string) {
	...
	rl, err := readline.NewEx(&readline.Config{
		Prompt:          prompt,
		HistoryFile:     filepath.Join(os.TempDir(), ".labs_history"),
		HistoryLimit:    100,
		InterruptPrompt: "^C",
		EOFPrompt:       "exit",
	})
