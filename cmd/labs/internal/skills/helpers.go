...
func skillsListBuiltinCmd() {
	...
	builtinSkillsDir := filepath.Join(filepath.Dir(cfg.WorkspacePath()), "labs", "skills")
...
func skillsSearchCmd(query string) {
	...
	for _, result := range results {
		...
		if result.RegistryName == "github" {
			fmt.Printf("     Install: labs skills install %s\n", result.Slug)
		} else {
			fmt.Printf("     Install: labs skills install --registry=%s %s\n", result.RegistryName, result.Slug)
		}
		fmt.Println()
	}
}
