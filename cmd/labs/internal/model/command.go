...
Examples:
  labs model                    # Show current default model
  labs model gpt-5.2           # Set gpt-5.2 as default
  labs model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  labs model local-model       # Set local VLLM server as default
  labs model add -b URL -k KEY # Add a model from a custom endpoint
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'labs model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'labs model add --help').")
}
