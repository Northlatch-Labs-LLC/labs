...
//
//	LABS_MCP_REAL_SERVER_JSON='{"enabled":true,"type":"http","url":"http://127.0.0.1:8080/mcp"}'
//
// Optional tool invocation:
//
//	LABS_MCP_REAL_TOOL_NAME=echo
//	LABS_MCP_REAL_TOOL_ARGS_JSON='{"message":"hello"}'
//	LABS_MCP_REAL_EXPECT_SUBSTRING=hello
//
// Stdio subprocess example:
//
//	LABS_MCP_REAL_SERVER_JSON='{"enabled":true,"type":"stdio","command":"npx","args":["-y","@modelcontextprotocol/server-filesystem","."]}'
func TestIntegration_RealConfiguredServer(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}

	serverJSON := strings.TrimSpace(os.Getenv("LABS_MCP_REAL_SERVER_JSON"))
	if serverJSON == "" {
		t.Skip("skipping integration test (set LABS_MCP_REAL_SERVER_JSON to enable)")
	}
	...
	if expectedCountRaw := strings.TrimSpace(os.Getenv("LABS_MCP_REAL_EXPECT_TOOL_COUNT")); expectedCountRaw != "" {
		expectedCount, err := strconv.Atoi(expectedCountRaw)
		if err != nil {
			t.Fatalf("invalid LABS_MCP_REAL_EXPECT_TOOL_COUNT %q: %v", expectedCountRaw, err)
		}
	...
	toolName := strings.TrimSpace(os.Getenv("LABS_MCP_REAL_TOOL_NAME"))
	if toolName == "" {
		return
	}

	toolArgs, err := loadRealToolArgs(os.Getenv("LABS_MCP_REAL_TOOL_ARGS_JSON"))
	if err != nil {
	...
	if want := os.Getenv("LABS_MCP_REAL_EXPECT_SUBSTRING"); want != "" && !strings.Contains(textPayload, want) {
