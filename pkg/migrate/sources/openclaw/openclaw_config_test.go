...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.labs/workspace" {
		t.Errorf("expected workspace '~/.labs/workspace', got '%s'", workspace)
	}
...
func TestConvertToLabs(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToLabs("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.labs/workspace" {
		t.Errorf("expected workspace '~/.labs/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToLabsWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToLabs("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToLabsWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToLabsWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.labs/workspace" {
		t.Errorf("expected workspace '~/.labs/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
