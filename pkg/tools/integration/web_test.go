...
// with cf-mitigated: challenge triggers a retry using the honest labs User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest labs user agent
	if !strings.Contains(receivedUAs[1], "labs") {
		t.Errorf("retry request UA = %q, want it to contain 'labs'", receivedUAs[1])
	}
}
