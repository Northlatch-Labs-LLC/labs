  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "labs-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_SessionDimensions(t *testing.T) {
	...
	// labs keeps no session across wakings, so no dimension is ever configured by default.
	if len(cfg.Session.Dimensions) != 0 {
		t.Errorf("Session.Dimensions = %v, want none", cfg.Session.Dimensions)
	}
}
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".labs", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	t.Setenv("LABS_HOME", "/custom/labs/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/labs/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf(
			"Workspace path with LABS_HOME = %q, want %q",
...
func TestLoadConfig_WarnsForPlaintextAPIKey(t *testing.T) {
	...
	t.Setenv("LABS_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("LABS_SSH_KEY_PATH", "")
...
// unchanged when LABS_KEY_PASSPHRASE is not set.
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when LABS_KEY_PASSPHRASE
