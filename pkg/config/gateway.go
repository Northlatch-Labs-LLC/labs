...
type GatewayConfig struct {
	Host      string `json:"host"                env:"LABS_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"LABS_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"LABS_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"LABS_LOG_LEVEL"`
}
...
// The LABS_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("LABS_LOG_LEVEL"); envLevel != "" {
