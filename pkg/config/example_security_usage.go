...
```bash
cp ~/.labs/config.json ~/.labs/config.json.backup
```

## Step 2: Create .security.yml
```bash
cp security.example.yml ~/.labs/.security.yml
```

## Step 3: Fill in your API keys
Edit ~/.labs/.security.yml and replace placeholders with your actual keys.

## Step 4: Simplify config.json (Recommended)
Remove sensitive fields from ~/.labs/config.json:
...
3. Set file permissions: chmod 600 ~/.labs/.security.yml
...
```bash
# Channels
export LABS_CHANNELS_TELEGRAM_TOKEN="token-from-env"
export LABS_CHANNELS_DISCORD_TOKEN="discord-token-from-env"

# Web Tools
export LABS_TOOLS_WEB_BRAVE_API_KEY="brave-key-from-env"
export LABS_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"

# Skills
export LABS_TOOLS_SKILLS_GITHUB_TOKEN="github-token-from-env"
```
...
- Verify the file permissions allow reading (chmod 600 ~/.labs/.security.yml)
