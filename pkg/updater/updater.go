...
func DownloadAndExtractRelease(releaseURL, platform, arch string) (string, error) {
	...
	// Extract
	destDir, err := os.MkdirTemp("", "labs-extract-*")
	if err != nil {
		os.Remove(tmpPath)
		return "", err
	}
...
// Example: owner="sky5454" -> https://api.github.com/repos/sky5454/labs/releases/latest
func GetReleaseAPIURL(owner string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/labs/releases/latest", owner)
}
...
// Example: owner="sipeed", tag="nightly" -> https://api.github.com/repos/sipeed/labs/releases/tags/nightly
func GetReleaseTagAPIURL(owner, tag string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/labs/releases/tags/%s", owner, tag)
}
