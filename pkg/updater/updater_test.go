...
const testReleaseAPIPath = "/api.github.com/repos/sipeed/labs/releases/latest"
...
func TestDownloadAndExtractRelease_IntegrationLatestRelease(t *testing.T) {
	if os.Getenv("LABS_INTEGRATION_TESTS") == "" {
		t.Skip("skipping integration test (set LABS_INTEGRATION_TESTS=1 to enable)")
	}
	if testing.Short() {
...
func TestFindAssetInfo_SelectsPreferredAsset(t *testing.T) {
		...
		case testReleaseAPIPath:
			writeReleasePayload(w, testReleasePayload{
				...
				Assets: []testReleaseAsset{
					{
						Name:               "labs_Linux_x86_64.zip",
						BrowserDownloadURL: server.URL + "/assets/labs_Linux_x86_64.zip",
						Digest:             "sha256:" + strings.Repeat("1", 64),
					},
					{
						Name:               "labs_Linux_x86_64.tar.gz",
						BrowserDownloadURL: server.URL + "/assets/labs_Linux_x86_64.tar.gz",
						Digest:             "sha256:" + strings.Repeat("2", 64),
					},
					{
						Name:               "labs_Windows_x86_64.zip",
						BrowserDownloadURL: server.URL + "/assets/labs_Windows_x86_64.zip",
						Digest:             "sha256:" + strings.Repeat("3", 64),
					},
					{
						Name:               "labs_Windows_arm64.zip",
						BrowserDownloadURL: server.URL + "/assets/labs_Windows_arm64.zip",
	...
	tests := []struct {
	...
	}{
		{
			...
			platform:     "linux",
			arch:         "amd64",
			wantURL:      server.URL + "/assets/labs_Linux_x86_64.tar.gz",
		...
		},
		{
			...
			platform:     "windows",
			arch:         "amd64",
			wantURL:      server.URL + "/assets/labs_Windows_x86_64.zip",
		...
		},
		{
			...
			platform:     "windows",
			arch:         "arm64",
			wantURL:      server.URL + "/assets/labs_Windows_arm64.zip",
...
func TestFindAssetInfo_UsesChecksumAssetWhenDigestMissing(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/assets/checksums.txt":
			_, _ = io.WriteString(w, checksum+"  labs_Windows_x86_64.zip\n")
		case "/assets/labs_Windows_x86_64.zip":
	...
	if gotURL != server.URL+"/assets/labs_Windows_x86_64.zip" {
		t.Fatalf("assetURL = %q, want %q", gotURL, server.URL+"/assets/labs_Windows_x86_64.zip")
	}
...
func TestDownloadAndExtractRelease_ExtractsTarGz(t *testing.T) {
	tarGzContent := buildTestTarGz(t, map[string]string{
		"labs_Linux_x86_64/labs": "test linux binary payload",
	})
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/assets/labs_Linux_x86_64.tar.gz":
	...
	binPath, err := findBinaryInDir(dir, "labs")
	if err != nil {
...
func TestDownloadAndExtractRelease_RetriesTransientAssetFailure(t *testing.T) {
	zipContent := buildTestZip(t, map[string]string{
		"labs.exe": "test windows binary payload",
	})
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api.github.com/repos/sipeed/labs/releases/latest":
			w.Header().Set("Content-Type", "application/json")
			fmt.Fprintf(
				w,
				`{"tag_name":"v0.2.6","assets":[{"name":"labs_Windows_x86_64.zip","browser_download_url":%q,"digest":"sha256:%s"}]}`,
				server.URL+"/assets/labs_Windows_x86_64.zip",
				checksum,
			)
		case "/assets/labs_Windows_x86_64.zip":
			assetAttempts++
	...
	dir, err := DownloadAndExtractRelease(
		server.URL+"/api.github.com/repos/sipeed/labs/releases/latest",
		"windows",
		"amd64",
	)
	if err != nil {
	...
	bs, err := os.ReadFile(filepath.Join(dir, "labs.exe"))
	if err != nil {
		t.Fatalf("ReadFile extracted asset: %v", err)
	}
