...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via LABS_KEY_PASSPHRASE
...
//  2. LABS_SSH_KEY_PATH env var
//  3. ~/.ssh/labs_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "LABS_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading LABS_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "LABS_SSH_KEY_PATH"

// labsHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const labsHome = "LABS_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "labs-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (LABS_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// LABS_SSH_KEY_PATH env var or ~/.ssh/labs_ed25519.key.
...
//   - exact match with LABS_SSH_KEY_PATH env var
//   - within the LABS_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with LABS_SSH_KEY_PATH.
	...
	// Within LABS_HOME.
	if picoHome := os.Getenv(labsHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="labs-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set LABS_SSH_KEY_PATH or place key at ~/.ssh/labs_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (LABS_SSH_KEY_PATH, LABS_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. LABS_SSH_KEY_PATH env var
//  3. ~/.ssh/labs_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the labs-specific SSH key path if it exists.
